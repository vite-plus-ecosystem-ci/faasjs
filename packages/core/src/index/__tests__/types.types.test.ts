...
it('FuncEventType and FuncReturnType should infer from Func', () => {
...
it('FuncEventType should include schema params for defineApi', () => {
...
it('FuncEventType should keep wide params when schema is missing', () => {
  const func = defineApi({
    async handler() {
      return true
    },
...
it('defineApi handler data should include cookie and session', () => {
  const func = defineApi({
