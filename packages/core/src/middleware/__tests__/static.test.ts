...
describe('staticHandler', () => {
  ...
  it('should work', async () => {
    ...
    const response2 = await fetch(`http://127.0.0.1:${port}/use-middleware.api.ts`)
  ...
  it('notFound with fallback path', async () => {
    ...
    expect(await response.text()).toBe(
      readFileSync(join(apisRoot, 'use-middleware.api.ts'), 'utf-8'),
    )
  })
  ...
  it('should support no-cache mode without using static cache map', async () => {
    const response1 = await fetch(`http://127.0.0.1:${port}/no-cache-404`)
    expect(response1.status).toBe(404)
    expect(await response1.text()).toBe('Not Found')

    const response2 = await fetch(`http://127.0.0.1:${port}/no-cache-404`)
    expect(response2.status).toBe(404)
    expect(await response2.text()).toBe('Not Found')
  })
