...
describe('testing helpers', () => {
  it('truncates public tables while preserving excluded tables', async () => {
    ...
    try {
      ...
      await sql`CREATE TABLE faasjs_pg_migrations (name TEXT PRIMARY KEY)`
      await sql`INSERT INTO users (name) VALUES ('Alice')`
      await sql`INSERT INTO faasjs_pg_migrations (name) VALUES ('keep me')`

      await resetTestingDatabase(sql, ['faasjs_pg_migrations'])

      expect(await sql`SELECT * FROM users`).toEqual([])
      expect(await sql`SELECT * FROM faasjs_pg_migrations`).toEqual([{ name: 'keep me' }])
    } finally {
      await sql.end()
